        ...
        for i in range(self.num_nodes):
            for j in range(i + 1, self.num_nodes):
                self.edges[i][j] = self.edges[j][i] = self.Edge(i, j, math.sqrt(
                    pow(self.nodes[i][0] - self.nodes[j][0], 2.0) + pow(self.nodes[i][1] - self.nodes[j][1], 2.0)), initial_pheromone)
...
if __name__ == '__main__':
    ...
    _alpha = 0.75  # коэф альфа
    _beta = 2.0   #  коэф бета
    _p = 0.5       # коэф испарения феромона
    _pheromone_deposit_weight = 10.0   #   коэф Q
    ...
    #_nodes = [(random.uniform(-400, 400), random.uniform(-400, 400)) for _ in range(0, 50)]  # раномное создание вершин
